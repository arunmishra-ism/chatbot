...
# bagw(sentence): This function will append 1 to a list variable ‘bag’ if the word is contained inside our
#  input and is also present in the list of words created earlier.
def bagw(sentence):
...
# Finally, we’ll initialize an infinite while loop that will prompt the user for an input and print the Chatbot’s response.
while True:
    message = input("You: ")
    ints = predict_class(message)
    res = get_response(ints, intents)

    # Clear previous input and response lines
    sys.stdout.write("\033[F")  # Move cursor up one line (clearing response line)
    sys.stdout.write("\033[K")  # Clear the line (removing input)
    
    # Print the response in a new line
    print("Bot: " + res)
